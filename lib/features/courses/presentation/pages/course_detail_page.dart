...
//                   final allSessions = sessionSnap.data ?? [];
//                   final now = DateTime.now();

//                   // Lọc ra các buổi học chỉ trong ngày hôm nay
...
//                   // Sắp xếp các buổi học theo thời gian bắt đầu
//                   todaySessions.sort(
...
//                         child: Center(
//                           child: Text('Không có buổi học nào hôm nay.'),
//                         ),
//                       ),
//                     );
//                   }
...
//                       // Xác định trạng thái thực tế dựa trên thời gian hiện tại
//                       final bool isFinished = session.endTime.isBefore(now);
//                       final bool isEnabled = !isFinished;
...
//                         // Thay đổi màu sắc nếu buổi học đã kết thúc
//                         color: isEnabled ? null : Colors.grey[300],
//                         child: ListTile(
//                           // Vô hiệu hóa ListTile nếu buổi học đã kết thúc
//                           enabled: isEnabled,
//                           leading: const Icon(Icons.event_note),
//                           title: Text(session.title),
//                           subtitle: Text(
//                             'Thời gian: ${DateFormat('HH:mm').format(session.startTime)} - ${DateFormat('HH:mm').format(session.endTime)}\nTrạng thái: $statusText',
//                           ),
//                           isThreeLine: true,
...
class _CourseDetailPageState extends State<CourseDetailPage> {
  ...
  Future<void> _showSessionSelectorAndStartAttendance() async {
    ...
    try {
      // === THAY ĐỔI QUAN TRỌNG: LẤY TẤT CẢ BUỔI HỌC, KHÔNG LỌC TRƯỚC ===
      final allSessionsOfCourse = await sessionService
          .sessionsOfCourse(widget.courseModel.id)
          .first;
      if (!mounted) return;

      final now = DateTime.now();

      // B1: Lọc ra tất cả các buổi học của ngày hôm nay từ danh sách đầy đủ
      final todaySessions = allSessionsOfCourse.where((session) {
      ...
      // B2: Sắp xếp các buổi học theo thời gian bắt đầu
      todaySessions.sort((a, b) => a.startTime.compareTo(b.startTime));

      if (todaySessions.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Không có buổi học nào được lên lịch cho hôm nay.'),
          ),
        );
        return;
      }
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: ListView.builder(
              shrinkWrap: true,
              itemCount:
                  todaySessions.length, // Dùng danh sách đã lọc và sắp xếp
              itemBuilder: (context, index) {
                final session = todaySessions[index];

                // B3: Xác định trạng thái và khả năng tương tác của từng buổi học
                // Buổi học được coi là "đang diễn ra" nếu thời gian hiện tại nằm trong khoảng bắt đầu và kết thúc
                final bool isOngoing =
                    now.isAfter(session.startTime) &&
                    now.isBefore(session.endTime);
                final bool isFinished = now.isAfter(session.endTime);

                // Chỉ cho phép nhấn vào khi buổi học đang thực sự diễn ra
                final bool isEnabled = isOngoing;
